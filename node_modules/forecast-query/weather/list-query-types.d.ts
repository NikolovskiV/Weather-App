...
export interface ListForecastQueries {
    ...
    temp: (type?: 'exact' | 'feel' | 'max' | 'min') => Promise<({
        value: number;
        date: Date;
    })[]>;
    pressure: (type?: 'default' | 'ground' | 'sea') => Promise<({
        value: number;
        date: Date;
    })[]>;
    humidity: () => Promise<({
     ...
     */
    percipitation: (mode?: 'rain' | 'snow', time?: '1h' | '3h') => Promise<({
        value: number;
        date: Date;
    })[]>;
    wind: (type?: 'speed' | 'degree' | 'gust') => Promise<({
        value: number;
        date: Date;
    })[]>;
    rain: (when?: '1h' | '3h') => Promise<({
        value: number;
        date: Date;
    })[]>;
    snow: (when?: '1h' | '3h') => Promise<({
        value: number;
        date: Date;
    })[]>;
    /**
